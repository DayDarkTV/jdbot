...
const db = new node_json_db_1.JsonDB('./data', true, true, '/');
if (!db.exists('/liferoll'))
    db.push('/liferoll', {});
const Client = new Discord.Client({ intents: [Discord.Intents.FLAGS.GUILDS, Discord.Intents.FLAGS.GUILD_MESSAGES] });
var auth = fs.readFile('../auth.json', 'utf8', (err, data) => {
    let hello = JSON.parse(data);
...
function onMessage(msg) {
    ...
    if (text.startsWith('jd.lifeRoll')) {
        if (!db.exists(`/liferoll/${msg.author.id}`))
            db.push(`/liferoll/${msg.author.id}`, { time: 0 });
        msg.channel.sendTyping();
        let daytime = 1000 * 60 * 60 * 24;
        if ((db.getObject(`/liferoll/${msg.author.id}/time`)) <= (Date.now() - daytime)) {
            let username = Client.user?.tag ?? 'jdbot';
            let _temp1 = random(1, 50);
            if (typeof _temp1 === 'boolean')
                _temp1 = 0;
            let _temp2 = random(1, 50);
            if (typeof _temp2 === 'boolean')
                _temp2 = 0;
            ...
            if (!soclose)
                db.push(`/liferoll/${msg.author.id}/time`, Date.now());
            ...
            const embed = new Discord.MessageEmbed()
                .setAuthor({ name: msg.author.tag, iconURL: `${msg.author.avatarURL({ dynamic: true })}` })
                .setTitle(`not yet~!`)
                .setDescription(`cannot use this command yet~ (24hr timer)`)
                .setFooter({ text: username, iconURL: `${Client.user?.avatarURL({ dynamic: true }) ?? 'https://cdn.discordapp.com/avatars/737365428078116955/d44b735761a65696f8ab108c5e25237d.webp'}` });
            msg.channel.send({ embeds: [embed] });
        }
    }
